...
- `968901d` - 新增 claude.md 開發文檔
- `88d9ef6` - 添加調查教練按鈕和建立問卷功能

### 2026-01-12 下午: UI 改進 - 調查教練和快速建立問卷

改進問卷功能的用戶體驗，讓創建問卷更加直觀：

#### 新增功能
- ✅ **調查教練按鈕**: 在 ChatPanel 教練選擇列表中添加「調查教練」
- ✅ **建立問卷按鈕**: 在 ProgressBoard 問卷區塊添加快速建立按鈕
- ✅ **CreateSurveyModal**: 可視化表單創建問卷（無需 AI）
- ✅ **自動開啟**: 創建問卷後自動在新標籤頁開啟

#### 使用方式
用戶現在有兩種方式創建問卷：
1. **與 AI 討論**: 切換到「調查教練」，讓 AI 協助設計問卷
2. **直接創建**: 點擊「建立問卷」按鈕，填寫表單快速創建

#### 技術實現
- 創建 `CreateSurveyModal` 組件（支援動態添加/刪除選項）
- 在 `ChatPanel` 添加 'survey' 到教練類型列表
- 實現 `handleCreateSurvey` 函數調用 Supabase API
- 添加 `onCreateSurvey` 回調到 ProgressBoard
