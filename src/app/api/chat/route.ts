...
🎯 **POV**：金融交易者需要即時的幫助，因為缺少即時幫助使他們在交易時感到不安

這個 POV 清楚指出了核心需求。讓我們繼續探討...
...
\`\`\`json:action
{
  ...
  "data": {
    "user": "金融交易者",
    "need": "即時的幫助",
    "insight": "缺少即時幫助使他們在交易時感到不安",
    "statement": "金融交易者需要即時的幫助，因為缺少即時幫助使他們在交易時感到不安"
  }
}
\`\`\`
...
\`\`\`json:action
{
  ...
  "data": {
    "title": "建立 AI 即時助手",
    "description": "提供 24/7 即時回答交易相關問題，降低交易者的焦慮感",
    "tags": ["AI", "即時支援"]
  }
}
\`\`\`
...
\`\`\`json:action
{
  ...
  "data": {
    "name": "聊天機器人介面原型",
    "description": "一個簡單的聊天視窗，用戶可以輸入問題並立即獲得 AI 回覆",
    "type": "low_fidelity",
    "features": ["即時問答", "友善介面", "歷史記錄"]
  }
}
\`\`\`

### 🔴 最重要的提醒
**每次記錄資料時，你必須同時輸出對話內容和 JSON action！**

觀察記錄（同理心階段）：
- category 可選：pain_point（痛點）、behavior（行為）、need（需求）、insight（洞察）

POV 記錄（定義階段）：
- 必須包含：user（使用者）、need（需求）、insight（洞察）、statement（完整陳述）

點子記錄（發想階段）：
- 必須包含：title（標題）、description（描述）
- 可選：tags（標籤陣列）

原型記錄（原型階段）：
- 必須包含：name（名稱）、description（描述）
- 可選：type（low_fidelity/medium_fidelity/high_fidelity）、features（功能陣列）
